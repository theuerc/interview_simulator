...
from .services import chat_gpt, text_to_speech, transcribe_audio_with_whisper, gpt_questions
...
def check_uploads():
    ...
    """
    latest_resume = UserFile.query.filter_by(user_id=current_user.id, file_name="Resume").order_by(UserFile.upload_date.desc()).first()
    latest_job_description = UserFile.query.filter_by(user_id=current_user.id, file_name="Job Description").order_by(UserFile.upload_date.desc()).first()
    
    if latest_resume and latest_job_description:
        return jsonify({"uploaded": True, "resume": latest_resume.file_content, "job_description": latest_job_description.file_content})
    else:
...
def transcribe():
    ...
    Returns:
    - A string representing the JSON response containing the transcribed text, the ChatGPT API response and the question asked.
    """
    ...
    if audio_file:
        ...
        return jsonify({'transcription': transcription, 'response': response, 'question': question})
    else:
